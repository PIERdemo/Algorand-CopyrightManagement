...
if __name__ == "__main__":
    ...
    fund_transaction = transaction.PaymentTxn(sp= client_parameters, sender=user_2.address, receiver=logicsign_account.address(), amt=1000000)
    ...
    stateless_tx = transaction.PaymentTxn(sp= client_parameters, sender=logicsign_account.address(), 
                                          receiver=user_2.address, amt=0, note=bytes.fromhex(root+old_root))

    # BUILD AND CREATE STATEFULL SMART CONTRACT
    build_app = app.build()
    build_app.export("./artifacts/merkletree")

    user1_app_client = beaker.client.ApplicationClient(
                client=algo_client,
                app=app,
                sender=user_1.address,
                signer=user_1.signer,
            )
    app_id, addr, txid = user1_app_client.create(deafult_root = BYTE_DEFAULT_ROOT)
    ...
    fund_transaction = transaction.PaymentTxn(sp= client_parameters, sender=user_2.address, receiver=addr, amt=1000000)
    tx_id = algo_client.send_transaction(fund_transaction.sign(user_2.private_key))
    transaction.wait_for_confirmation(algo_client, tx_id, 10)
